...
detectionSharpType detectionObstacleSharp() {
	...
	if (sharpMes > SEUIL_COLLISION_AVANT) {
            ...
            detectionSharp.obstacleInfo.x = curPos.x + cos(curPos.alpha/10)*DISTANCE_OBSTACLE_AVANT;
            detectionSharp.obstacleInfo.y = curPos.y + sin(curPos.alpha/10)*DISTANCE_OBSTACLE_AVANT;
