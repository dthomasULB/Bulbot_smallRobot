...
// NOTE !!!
// Il ajoute l'obstacle mais ne recalcul pas le parcours.
                ...
                if (EnvoiSharpActif) {
                    if (msTimerGet()-sharpLastTime >= 10) {
                        ...
                        if (detectionObstacleSharp().isObstacleDetected ){
                            ...
                            if (obstacle == PROP_IS_UNMOVABLE_OBSTACLE){
...
actionType choixAction() {
	...
	static unsigned int indiceAction = 0;		//<! définit l'action en cours
        ...
        if (indiceAction < nbActions && !STOPED_OBS ) {
		indiceAction++;
	}
        else{
    ...
    switch(infoAction.typeEtapeEnCours) {
        case ACTION_TRAJECTOIRE:
            if (detectionSharp.isObstacleDetected){
                trajectoryBasicAction(RESET);   // Faut "purger" Basictrajectory
                // currentAction(RESET_ACTION);
                nextAction.ptr2Fct = StopNowFct;
                nextAction.option = 0;
                STOPED_OBS=1;
                ...
                case STANDING:                                                               // si on est à l'arrêt et en erreur, ce doit être un obstacle (ou un patinage)
                   ...
                   // }
                  //  if(detectionSharp.isObstacleDetected){
                  //     propulsionAddObstacle(detectionSharp);
                  //    }
